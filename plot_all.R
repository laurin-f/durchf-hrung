plot_all<-function(data,event,save=F,name,height=9,width=7){
  library(ggplot2)
  library(gridExtra)
  library(grid)
  library(cowplot)
  ...
  bf_plot<-ggplot()+geom_line(data=subset(data,!is.na(theta)),aes(date,theta,col=as.factor(tiefe)),show.legend = F)+geom_rect(data=event,aes(xmin=start,xmax=stop,ymin = -Inf, ymax = Inf), alpha = 0.15,fill="blue")+
    labs(y=expression(theta*"  [VOl %]"),x="",col="tiefe")+  theme_classic()+scale_x_datetime(limits = range(data$date))
  
  p<-plot_grid(co2_plot,bf_plot,align = "v",ncol=1,rel_heights = c(2,1))
  
  if(length(data$lf)!=0){  
    lf_plot<-ggplot()+geom_line(data=subset(data,!is.na(lf)),aes(date,lf),show.legend = F)+geom_rect(data=event,aes(xmin=start,xmax=stop,ymin = -Inf, ymax = Inf), alpha = 0.15,fill="blue")+
    labs(y=expression("LF  ["*mu*"S * cm"^{-1}*"]"),x="")+  theme_classic()+scale_x_datetime(limits = range(data$date))
    p<-plot_grid(co2_plot,bf_plot,lf_plot,align = "v",ncol=1,rel_heights = c(2,1,1))
  }

  if(length(data$q)!=0){ 
      q_plot<-ggplot()+geom_line(data=subset(data,!is.na(q)),aes(date,q),show.legend = F)+geom_rect(data=event,aes(xmin=start,xmax=stop,ymin = -Inf, ymax = Inf), alpha = 0.15,fill="blue")+
    labs(y=expression("q  [ml * min"^{-1}*"]"),x="")+  theme_classic()+scale_x_datetime(limits = range(data$date))
